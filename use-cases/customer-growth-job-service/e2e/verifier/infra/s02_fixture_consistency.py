...
def _verify(_context: RunContext) -> VerificationResult:
    ...
    authorization_verifier = (
        DEPLOY_DIR.parent / "verifier/authz/other/s26_gateway_authorization.py"
    ...
    expected_mock_endpoints = {
        **mock_endpoints,
        "/healthz": "get",
        "/fault/ethereum/<chain_id>/timeout": "post",
    }
    if actual_mock_endpoints != expected_mock_endpoints:
        errors.append(
            "mock service routes must contain only OAuth-like provider contracts, "
            f"healthz, and the explicit RPC timeout fault: actual={sorted(actual_mock_endpoints)}"
        )
    wire_contracts = (
