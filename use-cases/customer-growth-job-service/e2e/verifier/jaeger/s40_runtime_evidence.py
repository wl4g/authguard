"""Jaeger s40: verify DB, logs, metrics, cache, and runtime health."""
...
class RuntimeEvidenceVerifier(BaseVerifier):
    ...
    def _verify_observability_logs(self) -> None:
        ...
        self.details.append(
            ...
            "records with explicit provider normalization, protocol-neutral account linking, "
            "ext_authz Check, and opaque scope-resolution lifecycle events"
        )
    ...
    def _verify_postgresql_schema_isolation(self) -> None:
        ...
        sql = (
            ...
            "(SELECT count(*) FROM authguard.iam_standalone_credential WHERE "
            "kind='password' AND revoked_at IS NULL) || '|' || "
            "(SELECT count(*) FROM authguard.iam_standalone_credential WHERE "
            "kind='totp' AND revoked_at IS NULL) || '|' || "
            "(SELECT count(*) FROM authguard.iam_standalone_credential WHERE "
            "kind='webauthn' AND revoked_at IS NULL) || '|' || "
            "(SELECT count(DISTINCT split_part(subject, ':', 1)) FROM "
            "authguard.iam_principal_identity WHERE provider='wallet' AND "
            "subject ~ '^(eip155|solana|bip122):[^:]+:.+$') || '|' || "
            "(SELECT count(*) FROM authguard.iam_principal_identity WHERE "
            "provider='standalone' AND subject NOT LIKE 'local\\_%' ESCAPE '\\')"
        )
        isolation = self._postgresql_query(pod, sql)
        try:
            values = tuple(int(value) for value in isolation.split("|"))
        except ValueError as error:
            raise RuntimeError(
                f"PostgreSQL IAM/schema contract returned malformed counts: {isolation!r}"
            ) from error
        fixed = values[:4] + values[5:9] + values[10:11]
        if (
            len(values) != 16
            or fixed != (5, 5, 0, 7, 6, 2, 4, 6, 4)
            or values[4] < 13
            or values[9] < 3
            or min(values[11:14]) < 1
            or values[14:] != (3, 0)
        ):
            raise RuntimeError(
                "PostgreSQL IAM/schema semantic contract mismatch: expected fixed "
                "schema/policy fixtures, at least thirteen identities, all three active "
                "standalone credential kinds, all three CAIP namespaces, and no email-like "
                "standalone subject; "
                f"got {isolation!r}"
            )
        self.details.append(
            ...
            "four actions, six role bindings, durable external-identity bindings across "
            "Keycloak/LDAP/SCIM/social/standalone/wallet Providers, password/TOTP/WebAuthn "
            "credentials, canonical EVM/Solana/Bitcoin CAIP subjects, and no email-derived "
            "standalone subject"
        )
