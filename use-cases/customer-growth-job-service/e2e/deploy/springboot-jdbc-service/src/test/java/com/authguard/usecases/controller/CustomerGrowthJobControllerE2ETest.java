...
import com.authguard.usecases.repository.CustomerGrowthJobJdbcRepository;
...
class CustomerGrowthJobControllerE2ETest {
      ...
      try (TestApplication application = newTestApplication()) {
        ...
        try {
          ...
          if (!scenario.expectedAllowed()) {
            assertDeniedMutationDidNotChangeDatabase(application, scenario);
          }
          ...
          if (scenario.expectedStatus() != null && !scenario.expectedStatus().isBlank()) {
            assertThat(result.status()).as("scenario %s", scenario.id()).isEqualTo(scenario.expectedStatus());
          }
        } finally {
          accessScope.close();
        }
  ...
  private static ScenarioResult executeScenario(
      TestApplication application, AuthorizationScenario scenario) {
    try {
      return switch (scenario.operation()) {
        ...
        case "get" ->
            new ScenarioResult(
                application.controller().getJob(scenario.targetJobId()).getStatusCode()
                    == HttpStatus.OK,
                List.of(),
                null);
        case "create" -> {
        ...
        case "delete" -> {
          application.controller().deleteJob(scenario.targetJobId());
          Integer remaining =
              application
                  .jdbcTemplate()
                  .queryForObject(
  ...
  private static void assertDeniedMutationDidNotChangeDatabase(
      TestApplication application, AuthorizationScenario scenario) {
  ...
  private static TestApplication newTestApplication() {
    SingleConnectionDataSource dataSource =
        new SingleConnectionDataSource("jdbc:sqlite::memory:", true);
    E2EFixtures.seedCustomerGrowthJobs(dataSource);
    JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
    CustomerGrowthJobJdbcRepository repository = new CustomerGrowthJobJdbcRepository(jdbcTemplate);
    CustomerGrowthJobService service = new CustomerGrowthJobService(repository);
    return new TestApplication(
        dataSource, jdbcTemplate, new CustomerGrowthJobController(service));
  }
  ...
  private record TestApplication(
      ...
      CustomerGrowthJobController controller)
