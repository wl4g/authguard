...
//!
//! The verifier is optional: without `RESIGN_JWT_PUBLIC_KEY_FILE` the filter
...
const RESIGN_JWT_PUBLIC_KEY_FILE_ENV: &str = "RESIGN_JWT_PUBLIC_KEY_FILE";
...
#[derive(Clone)]
pub struct ResignJwtVerifier {
    public_key: RsaPublicKey,
}

impl ResignJwtVerifier {
    /// Builds the verifier from `RESIGN_JWT_PUBLIC_KEY_FILE`; `None` disables
    ...
    pub fn from_env() -> anyhow::Result<Option<Self>> {
        let Some(path) = std::env::var(RESIGN_JWT_PUBLIC_KEY_FILE_ENV)
            .ok()
        ...
        let pem = std::fs::read_to_string(&path).map_err(|error| {
            anyhow::anyhow!("read {RESIGN_JWT_PUBLIC_KEY_FILE_ENV} file {path}: {error}")
        })?;
        let public_key = RsaPublicKey::from_pkcs1_pem(&pem).map_err(|error| {
            anyhow::anyhow!("{RESIGN_JWT_PUBLIC_KEY_FILE_ENV} is not a PKCS#1 public PEM: {error}")
        })?;
    ...
    pub fn verify(&self, request: &Request<Body>) -> anyhow::Result<()> {
        ...
        tracing::info!(
            authguard.resign_jwt.subject = subject,
            authguard.resign_jwt.expires_in_seconds = expires.saturating_sub(now),
...
mod tests {
    ...
    fn bearer_prefix_is_stripped() {
        assert_eq!(
            ResignJwtVerifier::bearer_prefix_stripped(&HeaderValue::from_static("Bearer token")),
            Some("token")
        );
        assert_eq!(
            ResignJwtVerifier::bearer_prefix_stripped(&HeaderValue::from_static("Bearer")),
            None
        );
        assert_eq!(
            ResignJwtVerifier::bearer_prefix_stripped(&HeaderValue::from_static("Basic abc")),
            None
        );
    }
    ...
    fn compact_token_requires_exactly_three_parts() {
        assert!(ResignJwtVerifier::compact_parts("a.b.c").is_ok());
        assert!(ResignJwtVerifier::compact_parts("a.b").is_err());
        assert!(ResignJwtVerifier::compact_parts("a.b.c.d").is_err());
        assert!(ResignJwtVerifier::compact_parts(".b.c").is_err());
        assert!(ResignJwtVerifier::compact_parts("a.b.").is_err());
    }
}
