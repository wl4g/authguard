...
impl PasswordVerifier {
    pub(super) fn new(minimum_length: usize) -> anyhow::Result<Self> {
        Ok(Self { minimum_length, dummy_hash: hash_sync(DUMMY_PASSWORD)? })
    }

    pub(super) fn validate_new(&self, password: &str) -> bool {
        password.len() >= self.minimum_length && password.len() <= 1024
    }

    pub(super) async fn hash(&self, password: String) -> anyhow::Result<String> {
        tokio::task::spawn_blocking(move || hash_sync(&password)).await?
    }

    pub(super) async fn verify(&self, password: String, encoded_hash: Option<String>) -> bool {
