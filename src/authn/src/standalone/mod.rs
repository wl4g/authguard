...
use self::password::PasswordVerifier;
use self::totp::TotpService;
use self::webauthn::WebauthnService;
pub(crate) use self::webauthn::{
...
pub(crate) struct StandaloneHandler {
    config: StandaloneAuthnProperties,
    credentials: Arc<dyn StandaloneCredentialRepository>,
...
impl StandaloneHandler {
    pub(crate) fn open(runtime: &AuthnRuntime) -> anyhow::Result<Option<Self>> {
        ...
        if (config.totp.enabled || config.webauthn.enabled) && challenges.is_none() {
            anyhow::bail!("standalone TOTP/WebAuthn requires the Redis challenge store");
        }
        let totp = config
            .totp
            .enabled
            ...
            .transpose()?;
        let webauthn =
            config.webauthn.enabled.then(|| WebauthnService::new(&config.webauthn)).transpose()?;
        Ok(Some(Self {
    ...
    fn challenge_store(&self) -> Result<&dyn ChallengeStore, ApiError> {
        self.challenges
            .as_deref()
...
pub(crate) async fn register(
    ...
    let subject = format!("local_{}", random_challenge_id());
    ...
    let session = if headers.contains_key(axum::http::header::AUTHORIZATION) {
        let principal_id =
            state.pipeline.authenticate_session(&headers).map_err(ApiError::session)?;
    ...
    state
        ...
        .create_credential(&IamStandaloneCredential {
            id: format!("pwd_{}", random_challenge_id()),
            identity: identity
                .key()
...
pub(crate) async fn totp_enrollment_challenge(
    ...
    let credential_id = format!("totp_{}", random_challenge_id());
...
fn normalize_login(login: &str) -> Result<String, ApiError> {
