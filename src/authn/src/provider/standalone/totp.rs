...
#[derive(Clone)]
pub(crate) struct TotpService {
    config: TotpAuthnProperties,
    cipher: Aes256Gcm,
}

impl TotpService {
    pub(crate) fn new(config: TotpAuthnProperties, encryption_key: &str) -> anyhow::Result<Self> {
        let key =
    ...
    pub(crate) fn generate(&self, account_name: &str) -> anyhow::Result<(String, String)> {
    ...
    pub(crate) fn encrypt(&self, plaintext: &str, credential_id: &str) -> anyhow::Result<String> {
    ...
    pub(crate) fn decrypt(&self, encoded: &str, credential_id: &str) -> anyhow::Result<String> {
        let payload = encoded
            .strip_prefix("v1:")
