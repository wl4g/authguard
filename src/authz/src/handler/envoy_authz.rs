...
use crate::model::{
    ...
    Effect, EvaluationContext, IamRoleInfo, UrnPattern,
};
...
#[derive(Debug, Clone)]
pub(crate) struct CompiledRoleBinding {
    pub(crate) id: String,
    pub(crate) principal_id: String,
    pub(crate) role_id: String,
    pub(crate) effect: Effect,
    pub(crate) resource_urn: UrnPattern,
    pub(crate) conditions: AuthorizationConditions,
}

/// Immutable evaluator built from the active `IamRoleInfo` and `IamRoleBindingInfo` catalog.
#[derive(Debug, Clone, Default)]
pub(crate) struct AuthorizationEvaluator {
    roles: HashMap<String, IamRoleInfo>,
    role_bindings: Vec<CompiledRoleBinding>,
}

impl AuthorizationEvaluator {
    pub(crate) fn new(roles: Vec<IamRoleInfo>, role_bindings: Vec<CompiledRoleBinding>) -> Self {
        Self {
...
pub struct DefaultAuthorizationHandler {
    ...
    resign_key: Option<resign::JwtSigningKey>,
}
...
impl DefaultAuthorizationHandler {
    ...
    async fn allowed(&self, started: Instant, evaluation: RequestEvaluation) -> CheckResponse {
        ...
        let resign_token = self.resign_token(&identity, &principal_id);
...
impl DefaultAuthorizationHandler {
    ...
    async fn evaluate_request(
        ...
        let evaluator = catalog.evaluator();
    ...
    fn resign_token(
        ...
        let now = epoch_seconds();
        let claims = Self::resign_token_claims(identity, principal_id, now, &self.scope_delivery);
        ...
        tracing::debug!(
            ...
            authguard.resign_token.ttl_seconds = self.scope_delivery.scope_token_ttl.as_secs(),
    ...
    pub(crate) fn resign_token_claims(
        ...
        for (name, value) in &identity.trusted_claims {
...
mod tests {
    ...
    use crate::config::ScopeDeliveryProperties;
