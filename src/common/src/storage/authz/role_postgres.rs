...
#[derive(Debug)]
pub struct AuthzPostgresRepository {
...
impl AuthzPostgresRepository {
    /// Opens the PostgreSQL-backed `AuthZ` aggregate repository.
    ///
    /// # Errors
    ///
...
impl Deref for AuthzPostgresRepository {
...
impl AuthzPostgresRepository {
    async fn load_actions(connection: &mut PgConnection) -> anyhow::Result<Vec<IamActionInfo>> {
    ...
    async fn load_roles(connection: &mut PgConnection) -> anyhow::Result<Vec<IamRoleInfo>> {
...
#[async_trait]
impl PolicyRepository for AuthzPostgresRepository {
