...
export function PolicyPage() {
  ...
  return <section>
    ...
    <div className="tabbar">{(Object.keys(definitions) as ResourceType[]).map(item => <button className={type === item ? 'active' : ''} onClick={() => setType(item)} key={item}>{labels[item]}</button>)}</div>
    <div className="data-card">
      <div className="data-toolbar"><span>{collection.total} {labels[type].toLowerCase()}</span><button className="primary small" disabled={!token} onClick={() => setEditing({ ...definition.initial })}><Plus/>{t('add')}</button></div>
...
function JsonDialog({ title, value, isNew, onClose, onSave }: { title: string; value: Resource; isNew: boolean; onClose: () => void; onSave: (value: Resource, original: Resource | null) => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><div className="modal"><header><div><span className="eyebrow">JSON RESOURCE</span><h3>{title}</h3></div><button onClick={onClose}><X/></button></header><label>{t('json')}<textarea rows={17} spellCheck={false} value={text} onChange={event => setText(event.target.value)}/></label>{error && <div className="error-banner">{error}</div>}<footer><button onClick={onClose}>{t('cancel')}</button><button className="primary" disabled={busy} onClick={submit}>{t('save')}</button></footer></div></div>
}
