...
export function PrincipalPage() {
  ...
  return <section>
    ...
    <div className="data-card"><div className="data-toolbar"><div className="search-box"><Search/><input value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => event.key === 'Enter' && load()} placeholder={t('search')}/></div><div><button onClick={() => setMode('federated')}><CloudDownload/>{t('federatedSearch')}</button><button className="primary small" onClick={() => setMode('local')}><Plus/>{t('localCreate')}</button></div></div>
      {error && <div className="error-banner">{error}</div>}
      {!page.items.length ? <div className="empty"><UserRound/><p>{token ? t('empty') : t('controlToken')}</p></div> : <div className="principal-table"><div className="table-head"><span>{t('principalId')}</span><span>{t('displayName')}</span><span>{t('kind')}</span><span>{t('status')}</span><span/></div>{page.items.map(principal => <div className="table-row" key={principal.id}><code>{principal.id}</code><strong>{principal.display_name}</strong><span>{principal.kind}</span><span className={`status ${principal.status.toLowerCase()}`}>{principal.status}</span><div className="row-actions"><button title={principal.status === 'ACTIVE' ? t('disabled') : t('active')} onClick={() => status(principal)}>{principal.status === 'ACTIVE' ? <ToggleRight/> : <ToggleLeft/>}</button><button className="danger" onClick={() => remove(principal)}><Trash2/></button></div></div>)}</div>}
    </div>
    {mode === 'local' && (
...
function LocalAccountDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><form className="modal narrow" onSubmit={submit}><header><div><span className="eyebrow">STANDALONE</span><h3>{t('localCreate')}</h3></div><button type="button" onClick={onClose}><X/></button></header><label>{t('username')}<input value={login} onChange={event => setLogin(event.target.value)} required/></label><label>{t('displayName')}<input value={displayName} onChange={event => setDisplayName(event.target.value)} required/></label><label>{t('password')}<input type="password" value={password} onChange={event => setPassword(event.target.value)} required/></label>{error && <div className="error-banner">{error}</div>}<footer><button type="button" onClick={onClose}>{t('cancel')}</button><button className="primary" disabled={busy}>{t('createAccount')}</button></footer></form></div>
}
...
function FederatedDialog({ token, onClose, onCreated }: { token: string; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><div className="modal"><header><div><span className="eyebrow">FEDERATED DISCOVERY</span><h3>{t('federatedSearch')}</h3></div><button onClick={onClose}><X/></button></header><form className="inline-search" onSubmit={search}><input value={text} onChange={event => setText(event.target.value)} required placeholder={t('search')}/><button className="primary" disabled={busy}><Search/></button></form><div className="discovery-list">{results.map(result => <button className={selected === result ? 'selected' : ''} onClick={() => { setSelected(result); setPrincipalId(`P_${result.reference.external_id}`) }} key={`${result.reference.issuer}:${result.reference.external_id}`}><strong>{result.display_name}</strong><span>{result.email || result.username || result.reference.external_id}</span><small>{result.reference.provider_id}</small></button>)}</div>{selected && <label>{t('principalId')}<input value={principalId} onChange={event => setPrincipalId(event.target.value)}/></label>}{error && <div className="error-banner">{error}</div>}<footer><button onClick={onClose}>{t('cancel')}</button><button className="primary" disabled={!selected || !principalId || busy} onClick={materialize}>{t('materialize')}</button></footer></div></div>
}
