...
- Envoy Gateway 负责统一入口、TLS、路由、canonical JWT 验证和 **PEP(Policy Enforcement Point)**；
- `authguard-authn` 负责 Provider 协议适配、`ExternalIdentity` 规范化和 Account Linking；
- `authguard-authz` 负责仅围绕内部稳定 `principal_id` 并联合后端 Business microservices (内嵌 authguard adapters sdk) 解析 URNs 并作为 SQL Query where 条件对数据/资源级实现鉴权访问控制, 实现 **PDP(Policy Decision Point)**。
...
- 可选的 LDAP/Keycloak pull discovery，以及与之互补的 SCIM push provisioning。

管理面 pull connector 不参与登录或热路径。管理员物化候选时写入 canonical `principal_id` 与 identity binding；SCIM 随后把生命周期变化 push 到同一 Principal 投影。

## 协议边界

- GitHub OAuth 不是 OIDC；通常使用 access token 调用 GitHub `/user` 获取身份；
- ID Token 不等于 UserInfo；Access Token 也不等于用户身份；
- 标准 OIDC discovery/callback/ID Token/UserInfo 统一由 AuthN 处理；
